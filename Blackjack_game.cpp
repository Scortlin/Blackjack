...
public:
	enum rank {
		АСЕ = 1, TWO, THREE, FOUR, FIVE, SIX, SEVEN, EIGHT, NINE, TEN, JACK, QUEEN, KING
	};
...
class Hand {
	public:
		...
		int GetTotal() const; // получает сумму очков карт в руке
	protected:
		vector<Card*> m_Cards;
};
	Hand::Hand() {
	...
	int Hand::GetTotal() const {
		...
		for (iter = m_Cards.begin(); iter != m_Cards.end(); ++iter) {
			if ((*iter)->GetValue() == Card::rank::АСЕ) {
				containsAce = true;
			}
		}
...
class Player :public GenericPlayer {
	...
	bool Player::IsHitting() const {
		cout << m_Name << ". Хочешь взять карту? (Y/N): ";
		char response;
		cin >> response;
		return (response == 'y' || response == 'Y');
	}
	void Player::Win() const {
		cout << m_Name << " выиграл(а).\n";
	}
	void Player::Lose() const {
		cout << m_Name << " проиграл(а).\n";
	}
	void Player::Push() const {
		cout << m_Name << " сыграл в ничью.\n";
	}
...
class Deck :public Hand {
	...
	void Deck::Populate() {
		...
		for (int s = Card::CLUBS; s <= Card::SPADES; ++s) {
			for (int r = Card::rank::АСЕ; r <= Card::KING; ++r) {
